...
import com.ripenapps.greenhouse.databinding.ActivityBidderEditAddress2Binding
...
import com.ripenapps.greenhouse.screen.seller.GPSTracker
...
class BidderEditAddress : BaseActivity(), OnMapReadyCallback {
    private lateinit var binding: ActivityBidderEditAddress2Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        CommonUtils.changeStatusBarColor(this@BidderEditAddress, R.color.status_bar)
        if (!Places.isInitialized()) {
            Places.initialize(applicationContext, "AIzaSyCQH2QMt93mrbovh5mCHkxa7a5ROo0Clns")
        }
        binding = DataBindingUtil.setContentView(
            this@BidderEditAddress, R.layout.activity_bidder_edit_address2
        )

        if (Preferences.getStringPreference(this@BidderEditAddress, "language").equals("ar")) {
            binding.btnBackEditAdd.rotation = 180f
        }

        comingFrom = intent.getStringExtra("fromEditAddress") ?: ""
        binding.apply {
            address.setHint(getString(R.string.enter_address))
            address.setTitle(getString(R.string.confirm_address))
            address.setHint(getString(R.string.enter_address))
            address.setInputType(false, InputType.TYPE_CLASS_TEXT)
            address.setWidth()
            address.setTypeFace(
                this@BidderEditAddress, R.font.euclid_circular_regular
            )
            address.setImeAction("next")
//            address.isEnable(false)
            address.setTextColor(R.color.black)

            name.setTitle(getString(R.string.full_name))
            name.setHint(getString(R.string.enter_full_name))
            ...
            mobileNumberComplete.setTitle(getString(R.string.mobile_number))
            mobileNumberComplete.setHint(getString(R.string.enter_mobil))
            mobileNumberComplete.setInputType(false, InputType.TYPE_CLASS_NUMBER)
            mobileNumberComplete.setWidth()
            mobileNumberComplete.setTypeFace(
                this@BidderEditAddress, R.font.euclid_circular_medium
            )
    ...
    private fun validationObserver() {
        binding.apply {
            mobileNumberComplete.getValidatedValue().observe(this@BidderEditAddress) { validated ->
                isNumber = validated
...
//            showCustomToast("You need to enable the location permission.!")
        ...
        val locationCallback: LocationCallback = object : LocationCallback() {
            override fun onLocationResult(locationResult: LocationResult) {
                ...
                if (mostRecentLocation != null) {
                    if (isFinishing) {
                        LocationServices.getFusedLocationProviderClient(this@BidderEditAddress)
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == permissionId) {
            if (grantResults.isNotEmpty() && grantResults.getOrNull(0) == PackageManager.PERMISSION_GRANTED) {
                ...
                if (!isFinishing && !permissionPopUpShowed) CommonUtils.showSettingLocationDialog(
                    this, "Location"
                )
            }
        }
    ...
    private fun addAddress() {
        ...
        profileAddressAddEditViewModel?.request?.userType = "bidder"
        ...
        profileAddressAddEditViewModel?.request?.token =
            Preferences.getStringPreference(this@BidderEditAddress, TOKEN)
    ...
    private fun initViewModel() {
        profileAddressAddEditViewModel =
            ViewModelProvider(this@BidderEditAddress)[ProfileAddressAddEditViewModel::class.java]
        binding.lifecycleOwner = this

        profileAddressAddEditViewModel?.getProfileAddressAddEditData()
            ?.observe(this@BidderEditAddress) {
                try {
                    when (it.status) {
                        Status.SUCCESS -> {
                            ...
                            when (data.status) {
                                200 -> {
                                    Log.d(
                                        "TAG", "responsenitViewModel: ${Gson().toJson(data.data)}"
                                    )
                                    if (intent.hasExtra("fromEditAddress")) {
                                        Toast.makeText(
                                            this@BidderEditAddress,
                                        ...
                                        Toast.makeText(
                                            this@BidderEditAddress, data.message, Toast.LENGTH_SHORT
                                        ).show()
                                ...
                                403, 401 -> {
                                    val signin = Intent(this@BidderEditAddress, SignIn::class.java)
                        ...
                        Status.ERROR -> {
                            ...
                            ProcessDialog.dismissDialog()
                            Toast.makeText(this@BidderEditAddress, it.message, Toast.LENGTH_SHORT)
                                .show()
                        }
                    }
                } catch (e: Exception) {
